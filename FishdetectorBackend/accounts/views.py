...
firebase_admin.initialize_app(cred, name='my-app-name', options={"storageBucket": "project28-f22ba.appspot.com"})
...
def predict(request):
    if request.method == "POST":
        ...
        result = model(file_path, save=True, project=predict_path, name=f"{filename[:-4]}")
        ...
        #destination_path = f"{new_filename[:-4]}"
        public_url = upload_video_to_firebase(video_path, destination_path, result_name)
...
def list_video_links(request):
    try:
        ...
        filtered_blobs = [blob for blob in blobs if blob.name.startswith(f"{username}_")]

        
        video_links = [{"url": blob.public_url, "filename": blob.name} for blob in filtered_blobs]
        ...
        for blob in blobs:
            if blob.name.startswith(f"{username}_"):
                ...
                if video_metadata is not None:
                    result_name = video_metadata.get("result_name", "")  # Get the result_name from metadata
                else:
                    result_name = ""  # Set a default value if metadata is None
                video_links.append({"url": blob.public_url, "filename": blob.name, "result_name": result_name})
